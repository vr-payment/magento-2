...
abstract class BaseOrderLifecycleHandler extends DefaultWebhookLifecycleHandler
{
     ...
     * non-VRPayment payment method: the Portal will eventually
    ...
    public function preProcess(WebhookListener $listener, WebhookContext $context): bool
    {
        ...
        if (!$this->sdkEntity) {
            $this->logger->debug(sprintf(
                'No SDK entity for %s %d — likely a Portal transaction abandoned because the order was placed with a non-VRPayment payment method. Acking webhook.',
        ...
        if (!$this->order) {
            $this->logger->debug(sprintf(
                'No Magento order linked to %s %d — likely a Portal transaction abandoned because the order was placed with a non-VRPayment payment method. Acking webhook.',
                $listener->getTechnicalName(),
                $context->entityId
            ));
            return false;
     ...
     * after a non-VRPayment checkout), we return only the base
    ...
    public function getLockableResources(WebhookListener $listener, WebhookContext $context): array
    {
        ...
        if ($orderId) {
            $locks[] = 'vrpayment_order_update_' . $orderId;
        }
        return $locks;
    }
