...
define([
    ...
    'VRPayment_Payment/js/model/checkout-handler',
    'VRPayment_Payment/js/vrpayment-sdk-loader',
    'mage/storage'
], function (
    $,
    Component,
    ...
    return Component.extend({
        defaults: {
            template: 'VRPayment_Payment/payment/form'
        },
        ...
        initialize: function () {
            ...
            if (window.checkoutConfig.vrpayment.integrationMethod == 'iframe') {
        ...
        fetchMetadata: function () {
            ...
            if (isGuest) {
                restUrl = '/V1/vrpayment/checkout/guest/' + quote.getQuoteId() + '/metadata/' + self.getCode();
            } else {
                restUrl = '/V1/vrpayment/checkout/mine/metadata/' + self.getCode();
            }
        ...
        createIframeHandler: function () {
            var self = this;
            var registry = window.__vrpaymentHandlers = window.__vrpaymentHandlers || {};
            ...
            if (this.handler) {
                ...
                this.fetchMetadata().done(function (response) {
                    ...
                    if (data.error) {
                        ...
                        fullScreenLoader.stopLoader(true);
                        console.error("VRPayment Metadata Error:", data.error);
                        return;
                    }
                    ...
                    sdkLoader.load(data.javascriptUrl).then(function () {
                        ...
                        if (typeof handlerFactory !== 'function') {
                            ...
                            $('body').trigger('processStop');
                            fullScreenLoader.stopLoader(true);
                            console.error("VRPayment SDK Load failed: IframeCheckoutHandler is not a function");
                            return;
                        }
                        ...
                        $('body').trigger('processStop');
                        fullScreenLoader.stopLoader(true);
                        console.error("VRPayment SDK Load failed", error);
                    });
                    ...
                    $('body').trigger('processStop');
                    fullScreenLoader.stopLoader(true);
                    console.error("VRPayment REST Metadata Call failed", error);
                });
            }
        },
        ...
        handleExpiredSession: function () {
            fullScreenLoader.stopLoader(true);
            $('body').trigger('processStop');
            window.location.replace(urlBuilder.build("vrpayment_payment/checkout/failure") + "?reason=expired");
        },
                ...
                this.getPlaceOrderDeferredObject()
                    .fail(
                        function (response) {
                            ...
                            if (typeof error == 'object' && error.message == 'vrpayment_checkout_failure') {
        ...
        afterPlaceOrder: function () {
            ...
            document.cookie = 'vrpayment_restore_pending=1; path=/; samesite=lax';
            ...
            if (!window.vrpaymentRestoreAttached) {
                window.vrpaymentRestoreAttached = true;
                var forceRestore = function () {
                    ...
                    if (document.cookie.indexOf('vrpayment_restore_pending=') === -1) {
                        return;
                    }
            ...
            fullScreenLoader.startLoader();
            $('body').trigger('processStart');
            if (window.checkoutConfig.vrpayment.integrationMethod == 'iframe' && this.handler) {
                this.handler.submit();
            } else if (window.checkoutConfig.vrpayment.integrationMethod == 'lightbox') {
                this.fetchMetadata().done(function (response) {
                    var data = JSON.parse(response);
                    if (data.error) {
                        console.error("VRPayment Lightbox Metadata Error:", data.error);
                    ...
                    console.error("VRPayment REST Metadata Call failed for Lightbox", error);
                    self.fallbackToPaymentPage();
                });
            } else {
        ...
        fallbackToPaymentPage: function () {
            ...
            if (window.checkoutConfig.vrpayment.paymentPageUrl) {
                ...
                window.location.assign(window.checkoutConfig.vrpayment.paymentPageUrl);
            } else {
                window.location.replace(urlBuilder.build("vrpayment_payment/checkout/failure"));
            }
        },
