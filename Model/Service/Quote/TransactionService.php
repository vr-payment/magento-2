...
class TransactionService extends AbstractTransactionService
{
    ...
    private function convertQuoteBillingAddress(Quote $quote)
    {
        ...
        if ($gdprEnabled) {
            ...
            // paying with a non-VRPayment method, the pending transaction left
            // on the Portal contains no PII. The VRPayment confirmation path
            // (Order/TransactionService::confirmTransaction) re-sends the full
            // address, so VRPayment customers are unaffected.
            $address->setCity('');
