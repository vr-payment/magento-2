...
use VRPayment\Payment\Model\Webhook\OrderInvoiceTrait;
...
class CaptureCommand extends WebhookCommand
{
    ...
    private function captureInvoice(Order $order, float $amount, ?InvoiceInterface $invoice): ?InvoiceInterface
    {
        ...
        if ($invoice instanceof InvoiceInterface) {
            ...
            // already set — which it isn't for $existingInvoice (loaded independently via
            // getInvoiceForTransaction(), not via $order->getInvoiceCollection()). Without this,
            // pay()'s internal $order->setTotalPaid(...) mutates that orphaned, never-saved
            // object instead of the $order this command actually persists: the invoice ends up
            // correctly marked Paid, but the order's total_paid silently stays at 0.
