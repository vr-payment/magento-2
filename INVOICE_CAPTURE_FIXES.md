...
| Method | Data key set |
|---|---|
| `setVRPaymentCapturePending` (CoreWebhook) | `white_label_machine_name_capture_pending` |
| `setVrpaymentCapturePending` (everywhere else) | `vrpayment_32_capture_pending` |

`SubmitQuote` sets `vrpayment_32_capture_pending = true` at checkout.
The admin invoice view (`Block/Adminhtml/Sales/Order/Invoice/View.php:44`) reads it back
with `getVrpaymentCapturePending()` (same key). The CoreWebhook clears
...
**Fix:** ✅ Applied — changed `setVRPaymentCapturePending` to
`setVrpaymentCapturePending` on line 210.
