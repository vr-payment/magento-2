...
{
    protected const WEBHOOK_PROGRESS_TABLE = 'vrpayment_webhook_progress';
    ...
    {
        return ['vrpayment_webhook_' . $listener->getTechnicalName() . '_' . $context->entityId];
    }
    ...
    public function onFailure(WebhookListener $listener, WebhookContext $context, \Throwable $exception): void
    {
        ...
        } elseif ($exception instanceof AbstractDomainException && $exception->isRetryable()) {
            // Expected, self-healing condition (e.g. a cross-entity ordering race) —
            // the portal will redeliver this webhook later, so this isn't an operator-facing error.
            $this->logger->info(
                "Webhook processing deferred for {$listener->getTechnicalName()} {$context->entityId}: " .
