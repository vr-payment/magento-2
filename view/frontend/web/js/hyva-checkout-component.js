    ...
    Alpine.data('hyvaCheckoutWhitelabelPaymentMethod', () => ({
        ...
        initComponent() {
            ...
            if (typeof hyvaCheckout !== 'undefined' && hyvaCheckout.validation) {
                hyvaCheckout.validation.register('payment:vrpayment_' + this.methodCode, () => {
        ...
        loadSdkAndInitialize() {
            const registry = window.__vrpaymentHandlers = window.__vrpaymentHandlers || {};
        ...
        createIframe() {
            if (this.handler && this.isIframeMode) {
                let containerId = 'vrpayment-iframe-' + this.methodCode;
        ...
        refreshIframe() {
            if (this.isIframeMode && this.handler) {
                const container = document.getElementById('vrpayment-iframe-' + this.methodCode);
                if (container) {
                    container.innerHTML = '';
                    this.createIframe();
                } else {
